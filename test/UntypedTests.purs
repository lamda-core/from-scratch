...
untypedTests = 
  suite "--== Untyped ==--" do
    ...
    suite "☯︎ Variable" do
      ...
      test "✅ x  Γ{x: x}  ∴  x" do
        reduce (Var "x") (Tuple "x" (Var "x") : Nil) # Assert.equal (Ok (Var "x"))
      test "✅ x  Γ{y: 1, x: y}  ∴  1" do
        reduce (Var "x") (Tuple "y" (Num 1.0) : Tuple "x" (Var "y") : Nil) # Assert.equal (Ok (Num 1.0))
    ...
    suite "☯︎ Application" do
      ...
      test "❌ x y  Γ{x: x}  ∴  Undefined variable: y" do
        reduce (App (Var "x") (Var "y")) (Tuple "x" (Var "x") : Nil) # Assert.equal (Err (UndefinedVar "y"))
      test "❌ 1 2  ∴  Not a function: 1" do
        reduce (App (Num 1.0) (Num 2.0)) Nil # Assert.equal (Err (NotAFunction (Num 1.0))) 
      test "✅ f 1  Γ{f: f}  ∴  f 1" do
        reduce (App (Var "f") (Num 1.0)) (Tuple "f" (Var "f") : Nil) # Assert.equal (Ok (App (Var "f") (Num 1.0)))
      test "✅ (λx. x) 1  ∴  1" do
